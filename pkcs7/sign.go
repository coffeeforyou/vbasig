...
// NewSignedMsData takes data and initializes a MS code signing struct that is
// ready to be signed via AddSigner. The digest algorithm is set to MD5 by default
// and can be changed by calling SetDigestAlgorithm.
func NewSignedMsData(marshalledData []byte) (*SignedData, error) {
	// content, err := asn1.Marshal(data)
	// if err != nil {
	// 	return nil, err
	// }
	// contentInfo expects marshalled data
	//marshalledData, _ = asn1.Marshal(1)
	ci := contentInfo{
		ContentType: OIDDataMsCodeSigning,
		Content:     asn1.RawValue{Class: 2, Tag: 0, Bytes: marshalledData, IsCompound: true},
	}
	sd := signedData{
		ContentInfo: ci,
		Version:     1,
	}
	var ud asn1.RawValue
	_, err := asn1.Unmarshal(marshalledData, &ud)
	if err != nil {
		return nil, err
	}
	// signedData expects raw data
	return &SignedData{sd: sd, data: ud.Bytes, digestOid: OIDDigestAlgorithmMD5}, nil
}
